...
class TestCanonicalizeStagedDataTool:
    ...
    def test_field_mapping(self, sample_dataframe):
        ...
        # Get the actual mapping that was used (from _get_mapping method)
        col_mapping = tool._get_mapping(list(sample_dataframe.columns))

        # Verify mapped fields exist in canonical data
        for staged_field, canonical_field in col_mapping.items():
    ...
    def test_additional_fields_preserved(self, sample_dataframe):
        ...
        # Add a field that won't match any canonical mapping patterns
        ...
        # Custom field should be preserved (normalized to snake_case)
        # The tool normalizes unmapped fields: 'custom_field' -> 'custom_field'
